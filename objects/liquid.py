...
class Liquid():
    ...
    def move(self, board):
        ...
        moves = []
         # check move left
        if left and board[self.y][self.x-1].mass < self.mass:
            moves.append((self.x-1, self.y))

        # check move right
        if right and board[self.y][self.x+1].mass < self.mass:
            moves.append((self.x+1, self.y))
        ...
        for i in range(1, self.wetness):
            left = left and self.x > i-1 and board[self.y][self.x-i].type != "solid"
